# AFTERFALL — 생존자들의 마지막 보루

> 탐사로 얻고, 파밍으로 욕심내고, 쉘터 방어로 대가를 치른다.

다크 서바이벌 매니지먼트 UI — 좌측 사이드바 내비게이션과 멀티 스크린(**쉘터 · 탐사 · 방어 · 생존자 · 타로 · 기록**), 상단 자원 바, 거친 분위기 테마로 구성됩니다. 모든 비주얼은 외부 이미지 의존 없이 CSS·이모지·인라인 SVG·절차적 아바타로 구현되어 실제 아트 에셋을 끼워넣을 슬롯도 마련돼 있습니다.
...
**생존자 합류**: 초반에는 **3명**으로 시작하고, 탐사 중 특정 이벤트(부상자 구조·은신 생존자 발견·방랑자 합류 등)로 새 생존자가 캠프에 합류해 최대 7명까지 늘어납니다. 새 멤버는 지친 상태로 도착하고 합류 시 사기가 오릅니다.
